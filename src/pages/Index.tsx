...
const Index = () => {
  ...
  const [btnLink, setBtnLink] = useState<string>(DEFAULT_LINK);
  const vagasRef = useRef<number>(VAGAS_INICIAL);
  const vagasTimeoutRef = useRef<NodeJS.Timeout | null>(null);
  const [carouselIndex, setCarouselIndex] = useState(0);

  useEffect(() => {
    const saved = localStorage.getItem("btn_link");
    if (saved) setBtnLink(saved);
  }, []);
        ...
        <div className="relative z-10 container mx-auto px-4 py-6">
          <div className="max-w-6xl mx-auto">
            ...
            <div className="text-center mb-6">
              <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold leading-snug mb-6 animate-slide-up max-w-3xl mx-auto uppercase tracking-wide">
                RECEBA ENTRADAS PRONTAS{" "}
                <span className="text-yellow-400">E DESCUBRA OS PADRÕES</span>
                {" "}QUE MAIS LUCRAM HOJE
              </h1>
              
              <div className="text-sm sm:text-base text-muted-foreground max-w-2xl mx-auto leading-relaxed animate-slide-up space-y-2 mb-6">
              ...
              <div className="max-w-md mx-auto animate-bounce-in">
                <a
                  ...
                  className="flex items-center justify-center gap-3 w-full bg-yellow-400 hover:bg-yellow-300 transition-colors duration-200 text-black font-black text-lg sm:text-xl uppercase rounded-xl py-4 px-6 shadow-strong"
                >
                  <Send className="w-5 h-5 fill-black stroke-none" />
                  <span className="flex flex-col items-start leading-tight">
                    <span>ENTRAR NO GRUPO AGORA</span>
                    <span className="text-xs font-semibold tracking-widest opacity-80">100% GRATUITO</span>
                  </span>
                </a>
              </div>
            </div>
